...
<head>
  ...
  <style>
    ...
    .head { background: #C45C14; color: #fff; padding: 1.25rem 1.5rem; }
    .body { padding: 1.5rem; }
...
<body>
  <div class="card">
    ...
    <div class="body">
      ...
      <div class="note">
        Este es un <strong>comprobante de pago</strong> de Fandez (acuse de recibo).
...
function issuePaymentVoucher({ request, client, amount }) {
  ...
  const code = `FANDEZ-${String(request.id).slice(0, 8).toUpperCase()}`;
...
function issueJobVoucher({ request, client, provider, financials }) {
  if (!request || !financials) return { error: 'Datos de cierre incompletos' };
  const existing = request.vouchers?.find((v) => v.phase === 'job_settlement');
  if (existing) return { voucher: existing, existing: true };
  ensureDir();
  const id = `vch-${uuidv4().slice(0, 12)}`;
  const voucher = {
    id,
    code: `TRABAJO-${String(request.id).slice(0, 8).toUpperCase()}`,
    phase: 'job_settlement',
    requestId: request.id,
    amount: financials.grandTotal,
    currency: 'CLP',
    clientName: client?.name || request.clientName || 'Cliente',
    clientEmail: client?.email || request.billingSnapshot?.invoiceEmail || null,
    providerName: provider?.name || request.technicianName || 'Socio Fandez',
    serviceName: request.serviceName,
    address: request.address,
    ...
    status: 'issued',
    breakdown: {
      visit: financials.visitPaid,
      service: financials.serviceAmount,
      materials: financials.materialsTotal,
      total: financials.grandTotal
    }
  };
  const b = voucher.breakdown;
  const html = `<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Voucher de trabajo ${voucher.code}</title>
  <style>body{font-family:system-ui;max-width:680px;margin:2rem auto;color:#111827;padding:0 1rem}.card{border:1px solid #E6E9EF;border-radius:16px;overflow:hidden}.head{background:#C45C14;color:#fff;padding:1.25rem 1.5rem}.body{padding:1.5rem}.row{display:flex;justify-content:space-between;gap:1rem;padding:.6rem 0;border-bottom:1px solid #F1F5F9}.total{font-size:1.3rem;font-weight:700;color:#059669}.note{margin-top:1rem;background:#F8FAFC;padding:1rem;border-radius:12px;font-size:12px;color:#475569}</style>
  </head><body><div class="card"><div class="head"><small>Voucher final del trabajo</small><h1>${company.name}</h1></div><div class="body">
  <p>${voucher.code} · ${formatDate(voucher.issuedAt)}</p>
  <div class="row"><span>Cliente</span><strong>${voucher.clientName}</strong></div>
  <div class="row"><span>Socio / ejecutor</span><strong>${voucher.providerName}</strong></div>
  <div class="row"><span>Servicio</span><span>${voucher.serviceName}</span></div>
  <div class="row"><span>Dirección</span><span>${voucher.address}</span></div>
  <div class="row"><span>Visita</span><span>${formatCLP(b.visit)}</span></div>
  <div class="row"><span>Servicio adicional</span><span>${formatCLP(b.service)}</span></div>
  <div class="row"><span>Materiales a costo</span><span>${formatCLP(b.materials)}</span></div>
  <p class="total">Total final: ${formatCLP(b.total)}</p>
  <div class="note">Este voucher acredita el cierre y desglose del trabajo. No reemplaza los documentos tributarios que correspondan a Fandez y al socio prestador.</div>
  </div></div></body></html>`;
  fs.writeFileSync(path.join(VOUCHER_DIR, `${id}.html`), html, 'utf8');
  return { voucher };
}
