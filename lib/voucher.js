...
function issueJobVoucher({ request, client, provider, financials }) {
  ...
  const voucher = {
    ...
    clientName: client?.name || request.clientName || 'Cliente',
    clientEmail: client?.email || request.billingSnapshot?.invoiceEmail || null,
    providerName: provider?.name || request.technicianName || 'Socio Fundez',
    serviceName: request.serviceName,
    address: request.address,
  ...
  <div class="note">Este voucher acredita el cierre y desglose del trabajo. No reemplaza los documentos tributarios que correspondan a Fundez y al socio prestador.</div>
  </div></div></body></html>`;
