/**
 * Offsite backup → GitHub (repo privado), cifrado AES-256-GCM.
 *
 * Env requeridos:
 *   BACKUP_GITHUB_ENABLED=true
 *   BACKUP_GITHUB_TOKEN=ghp_... (contenido: write al repo de backups)
 *   BACKUP_GITHUB_REPO=owner/fandez-backups
 *   BACKUP_GITHUB_ENCRYPT_KEY=frase-larga-secreta
 *
 * Opcionales:
 *   BACKUP_GITHUB_BRANCH=main
 *   BACKUP_GITHUB_RETENTION_DAYS=14
 *   BACKUP_GITHUB_PATH_PREFIX=backups
 */
...
function isGithubBackupEnabled() {
  return env('BACKUP_GITHUB_ENABLED').toLowerCase() === 'true'
    ...
    && Boolean(env('BACKUP_GITHUB_ENCRYPT_KEY'));
}
...
function getStatus() {
  return {
    ...
    configured: {
      ...
      encryptKey: Boolean(env('BACKUP_GITHUB_ENCRYPT_KEY')),
...
async function applyGithubRetention(repo, branch, prefix, retentionDays) {
  ...
  for (const type of dirs) {
    ...
    for (const entry of entries) {
      ...
      // Expect name like 2026-09-14-abcd1234.enc
...
/**
 * Publica snapshot cifrado (sin uploads) en el repo de backups.
 */
async function pushBackupToGithub({ manifest, snapshot }) {
  ...
  // GitHub Contents API ~100MB; con base64 ~75MB útiles.
  if (enc.length > 70 * 1024 * 1024) {
    return { error: 'Backup demasiado grande para GitHub Contents API (>70MB cifrado).' };
  }
  ...
  const base = `${prefix}/${type}/${date}-${shortId}`;
  const encPath = `${base}.enc`;
  const manPath = `${base}.manifest.json`;
  const safeManifest = buildSafeManifest(manifest);

  await putFile(repo, branch, encPath, enc, `backup(${type}): ${date} ${shortId}`);
  await putFile(
    repo,
    branch,
    manPath,
    Buffer.from(`${JSON.stringify(safeManifest, null, 2)}\n`, 'utf8'),
    `backup manifest(${type}): ${date} ${shortId}`
  );

  let removed = 0;
  try {
    removed = await applyGithubRetention(repo, branch, prefix, retentionDays);
  } catch (err) {
    console.warn('[backup-github] retención:', err.message);
  }
