...
const CONSENT_DEFINITIONS = {
  terminos: {
    ...
    required: true,
    legalBasis: 'contrato',
    purpose: 'Aceptación de Términos y Condiciones para usar la plataforma Fandez',
    revocable: false
  },
  privacidad: {
    type: 'privacidad',
  ...
  contrato_socio: {
    ...
    required: false,
    legalBasis: 'contrato',
    purpose: 'Contrato de prestación de servicios como socio Fandez',
    revocable: false
  }
};
...
function isGranted(body, field) {
  return body?.[field] === 'on' || body?.[field] === true || body?.[field] === 'true' || body?.[field] === 1;
}

function hasLegalConsent(body) {
  if (isGranted(body, 'consent_legal')) return true;
  return isGranted(body, 'consent_terminos') && isGranted(body, 'consent_privacidad');
}

function validateRegistrationConsents(body) {
  if (!hasLegalConsent(body)) {
...
function getRegistrationConsentPayload(body) {
  const legal = hasLegalConsent(body);
