...
const DEFAULT_TIMEOUT_MINUTES = getRequestTimeouts().unassignedNoticeMinutes;
let running = false;
...
async function createAlandNotice(store, request) {
  ...
  const message = await aland.addMessage({
    ...
    senderType: 'aland',
    senderName: 'Sofía',
    body: `Han pasado ${getRequestTimeouts().unassignedNoticeMinutes} minutos y aún no encontramos un socio para tu servicio de ${request.serviceName}. ¿Quieres que sigamos buscando? Puedes responder desde la app o desde el enlace del correo.`,
    meta: {
...
async function processRequest(store, io, request) {
  ...
  try {
    require('./agents/founderAlerts').notifyInstant({
      agent: 'Sofía',
      severity: 'high',
      title: `Sin socio · ${updated.serviceName || updated.id}`,
      body: [
        `Han pasado ${getRequestTimeouts().unassignedNoticeMinutes} min sin socio para el pedido.`,
        `Pedido: ${updated.id}`,
        `Cliente: ${client.name || client.email}`,
        `Comuna/dirección: ${updated.address || '—'}`
      ].join('\n'),
      dedupeKey: `no-provider-${updated.id}`
    }).catch(() => {});
  } catch (_) { /* ignore */ }
...
async function run(store, io, { timeoutMinutes = DEFAULT_TIMEOUT_MINUTES } = {}) {
  ...
  try {
      ...
      try {
        if (io) {
          ...
          io.to(`aland_client_${request.clientId}`).emit('client_open_request_alert', {
            ...
            body: Number(request.cancellationFeeCharged) > 0
              ? `Cerramos ${request.serviceName}: estuvo abierta demasiado tiempo sin técnico. Se retuvo ${request.cancellationFeeCharged} y se gestiona la devolución del resto.`
              : `Cerramos ${request.serviceName}: estuvo abierta demasiado tiempo sin técnico. Se gestiona la devolución completa.`,
            url: '/cliente',
    ...
    const timeouts = getRequestTimeouts();
    const techTimeoutMin = timeouts.techAcceptMinutes;
    const releasedTechs = typeof store.releaseStaleTechnicianAssignments === 'function'
      ? store.releaseStaleTechnicianAssignments(techTimeoutMin)
      : [];
    for (const item of releasedTechs) {
      try {
        const request = item.request;
        const enriched = store.enrichRequestForClient(request);
        try {
          require('./agents/founderAlerts').alertTechNoShow({
            request,
            reason: 'tech_accept_timeout',
        ...
        if (io) {
          io.to(`request_${request.id}`).emit(`request_update_${request.id}`, {
            request: enriched,
            techTimedOut: true,
            previousTechnicianId: item.previousTechnicianId
          });
          const providerSocket = store.providerSockets.get(request.providerId);
          if (providerSocket) {
            io.to(providerSocket).emit('provider_reassign_required', {
              requestId: request.id,
              serviceName: request.serviceName,
              previousTechnicianName: item.previousName,
              title: 'Técnico sin respuesta',
              body: `${item.previousName} no aceptó a tiempo. Asigna otro técnico o libera el pedido.`,
              url: '/proveedor/mando'
            });
          }
          io.to(`aland_client_${request.clientId}`).emit('client_open_request_alert', {
            type: 'tech_timeout',
            urgency: 'medium',
            title: 'Reasignando técnico',
            body: `El técnico no confirmó a tiempo. El socio está asignando otro para ${request.serviceName}.`,
            url: `/cliente/servicio/${request.serviceId}?tracking=${request.id}`,
            request: enriched
          });
        }
      } catch (err) {
        console.error(`[tech-accept-timeout] ${item.request?.id}:`, err.message);
      }
    }

    const reassignMin = timeouts.providerReassignMinutes;
    const releasedProviders = typeof store.releaseStaleProviderReassignments === 'function'
      ? store.releaseStaleProviderReassignments(reassignMin)
      : [];
    for (const item of releasedProviders) {
      try {
        const request = item.request;
        const enriched = store.enrichRequestForClient(request);
        try {
        ...
        if (io) {
          ...
          io.to(`aland_client_${request.clientId}`).emit('client_open_request_alert', {
            type: 'provider_reassign_timeout',
            urgency: 'high',
            title: 'Seguimos buscando un equipo',
            body: `El socio no asignó otro técnico a tiempo. La app continúa buscando para ${request.serviceName}.`,
            url: `/cliente/servicio/${request.serviceId}?tracking=${request.id}`,
            request: enriched
          });
          const previousSocket = store.providerSockets.get(item.previousProviderId);
          if (previousSocket) {
            io.to(previousSocket).emit('provider_job_released', {
              requestId: request.id,
              reason: 'reassign_timeout',
              title: 'Pedido liberado',
              body: 'No asignaste otro técnico a tiempo. El pedido volvió a búsqueda.',
              url: '/proveedor'
            });
          }
        }
      } catch (err) {
        console.error(`[provider-reassign-timeout] ${item.request?.id}:`, err.message);
      }
    }
