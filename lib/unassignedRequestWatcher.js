...
async function run(store, io, { timeoutMinutes = DEFAULT_TIMEOUT_MINUTES } = {}) {
  ...
  try {
    ...
    const techTimeoutMin = Math.max(3, parseInt(process.env.TECH_ACCEPT_TIMEOUT_MINUTES || '10', 10) || 10);
        ...
        if (io) {
          ...
          if (providerSocket) {
            io.to(providerSocket).emit('provider_reassign_required', {
              ...
              body: `${item.previousName} no aceptó a tiempo. Asigna otro técnico o deserta el pedido.`,
              url: '/proveedor/mando'
            });
          }
          io.to(`aland_client_${request.clientId}`).emit('client_open_request_alert', {
