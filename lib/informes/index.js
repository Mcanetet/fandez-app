...
function buildWeeklyFinanceReportWithDecisions(store, opts = {}) {
  const company = require('../../config/company');
  const base = buildWeeklyFinanceReport(store, opts);
  const clara = require('../agents/clara');
  return clara.attachDecisionPack(base, store, { appUrl: company.appUrl || 'https://www.fandez.cl' });
}

/** Campaña incorporación socios — Florencia (gráficas + horarios Chile) */
const PARTNER_HOOKS = [
  { hook: 'Pedidos listos. Tú eliges.', angle: 'Control del socio' },
  { hook: '¿Sigues cotizando fuera de la app?', angle: 'Dolor cotización' },
...
const MARKETING_ASSETS = [
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-ig-feed-hook.png',
  ...
  },
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-ig-carousel-01.png',
  ...
  },
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-ig-carousel-02.png',
  ...
  },
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-ig-carousel-03.png',
  ...
  },
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-story-tiktok-cta.png',
  ...
  },
  {
    ...
    url: '/uploads/marketing/campana-socios/fandez-linkedin-banner.png',
    use: 'LinkedIn banner',
    channels: ['linkedin']
  }
];
 ...
 * Campaña septiembre 2026: desde el lunes 7 en adelante.
 * @param {{ year?: number, month?: number, startDay?: number }} opts
 */
function buildPartnerMarketingCalendar({ year = 2026, month = 9, startDay } = {}) {
  const y = Number(year) || 2026;
  const m = Math.min(12, Math.max(1, Number(month) || 9));
  // Septiembre 2026: arranque lunes 7; otros meses desde el día 1
  const campaignStartDay = Number.isFinite(Number(startDay))
    ? Math.max(1, Number(startDay))
    : (y === 2026 && m === 9 ? 7 : 1);
  ...
  for (let day = 1; day <= dim; day++) {
    ...
    const active = day >= campaignStartDay;
    ...
    if (!active) {
      days.push({
        ...
        phase: 'prep',
        channel: null,
        channelLabel: 'Preparación',
        format: null,
        formatLabel: '—',
        postAt: null,
        postWhen: `${dateKey} · preparación (campaña desde el ${campaignStartDay})`,
        timezone: 'America/Santiago',
        campaign: `Incorporación socios · ${monthName}`,
        hook: 'Preparar piezas y cuentas',
        angle: 'Prep',
        caption: `Campaña socios Fandez empieza el ${y}-${String(m).padStart(2, '0')}-${String(campaignStartDay).padStart(2, '0')}.\nHoy: revisar creatividades, bio con link de registro y CRM.`,
        cta: 'Quiero ser socio',
        link: '/registro?role=provider',
        hashtags: ['#Fandez', '#SociosFandez'],
        storyText: '',
        asset: null,
        assetUrl: null,
        checklist: [
          'Revisar gráficas con logo oficial',
          'Actualizar bio IG con link registro socio',
          'Agendar publicaciones desde el día de arranque'
        ]
      });
      continue;
    }

    const hook = PARTNER_HOOKS[publishIndex % PARTNER_HOOKS.length];
    const slot = PARTNER_SLOTS[publishIndex % PARTNER_SLOTS.length];
    let effective = { ...slot };
    ...
    days.push({
      ...
      timezone: 'America/Santiago',
      campaign: `Incorporación socios · ${monthName} (desde el ${campaignStartDay})`,
      hook: hook.hook,
      angle: hook.angle,
      ...
      assetUrl: assetMeta?.url || `/uploads/marketing/campana-socios/${effective.asset}`,
      checklist: [
  ...
  const startKey = `${y}-${String(m).padStart(2, '0')}-${String(campaignStartDay).padStart(2, '0')}`;

  const today = new Date();
  const todayKey = today.toLocaleDateString('en-CA', { timeZone: 'America/Santiago' });
  let todayItem = days.find((d) => d.date === todayKey);
  if (!todayItem || !todayItem.active) {
    todayItem = activeDays.find((d) => d.date >= todayKey) || activeDays[0] || days[0];
  }

  const anchorKey = todayKey < startKey ? startKey : todayKey;
  const anchorMs = Date.parse(`${anchorKey}T12:00:00`);
  const weekEnd = anchorMs + 7 * 24 * 60 * 60 * 1000;
  let thisWeek = activeDays.filter((d) => {
    const t = Date.parse(`${d.date}T12:00:00`);
    return t >= anchorMs && t <= weekEnd;
  }).slice(0, 8);
  if (!thisWeek.length) thisWeek = activeDays.slice(0, 7);

  const charts = buildPartnerMarketingCharts(activeDays);

  const narrative = [
    `Florencia · Campaña incorporación socios · ${monthName}`,
    '',
    `Arranque: ${startKey} (lunes) en adelante. Días 1–${campaignStartDay - 1}: preparación.`,
    'Objetivo único: que empresas y profesionales se registren como socios.',
    `Próxima pieza (${todayItem?.date} · ${todayItem?.postAt || '—'} Chile): ${todayItem?.channelLabel} · “${todayItem?.hook}”.`,
    ...
    `Publicaciones del mes: ${activeDays.length}. Usa las gráficas para ver canal y hora.`
  ].join('\n');
...
function buildDayCopy(day, hook, channel) {
  ...
  if (day % 7 === 0) {
    caption += '\n\nEsta semana: activación asistida a cada registro.';
  }
...
async function buildInformesBundle(store, { date = new Date() } = {}) {
  ...
  const finance = buildWeeklyFinanceReportWithDecisions(store, { date });
  // Campaña socios: septiembre 2026 desde el lunes 7; luego mes actual
...
module.exports = {
  ...
  buildWeeklyFinanceReportWithDecisions,
