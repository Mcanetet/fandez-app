  ...
  return source
    .map((item, i) => {
      ...
      const specialtyIds = Array.isArray(item.specialtyIds)
        ...
        : [];
      const marketLowRaw = parseInt(item.marketLow, 10);
      const marketHighRaw = parseInt(item.marketHigh, 10);
      return {
        id,
        name,
        unit: String(item.unit || 'unidad').trim() || 'unidad',
        marketPrice,
        marketLow: Number.isFinite(marketLowRaw) && marketLowRaw >= 0
          ? marketLowRaw
          : Math.round(marketPrice * 0.9),
        marketHigh: Number.isFinite(marketHighRaw) && marketHighRaw >= 0
          ? marketHighRaw
          : Math.round(marketPrice * 1.25),
        specialtyIds,
