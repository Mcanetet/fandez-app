...
const backupGithub = require('./backupGithub');
...
const DATA_REQUEST_UPLOADS_ROOT = path.join(__dirname, '../data/uploads/requests');
const DATA_PROVIDER_UPLOADS_ROOT = path.join(__dirname, '../data/uploads/providers');
...
function ensureDirs() {
  try {
    ...
    console.warn('[Fandez Backup] No se pudo crear directorios locales:', err.message);
  }
}
...
async function loadConfigAsync() {
  if (backupStore.isAvailable()) {
    try {
      ...
      console.error('[Fandez Backup] loadConfigAsync MySQL:', err.message);
    }
  }
  return loadConfig();
}
...
async function saveConfig(updates) {
  ...
  try {
    ...
    console.warn('[Fandez Backup] No se pudo escribir config local:', err.message);
  }
  if (backupStore.isAvailable()) {
...
function validateSnapshot(snapshot) {
  ...
  if (data.app && data.app !== 'fandez') {
    throw new Error('El archivo no es un backup de Fandez');
  }
  if (
    ...
    throw new Error('El archivo no parece un backup de Fandez (falta users, services o requests)');
  }
  return data;
}
...
async function hydrateFromDatabase() {
  ...
  try {
    ...
    console.error('[Fandez Backup] Error hidratando desde MySQL:', err.message);
  }
}
async function importSnapshotFile(snapshot, triggeredBy = 'admin') {
  ...
  try {
    ...
    console.warn('[Fandez Backup] Backup importado sin copia en disco:', err.message);
  }
...
async function createBackup(store, type = 'manual', triggeredBy = 'admin') {
  ...
  if (backupStore.isAvailable()) {
    try {
      ...
      console.warn('[Fandez Backup] No se pudo incluir config en snapshot:', err.message);
    }
  }
  ...
  try {
    ...
    if (config.includeUploads) {
      ...
      uploadsBytes += copyDirRecursive(DATA_PROVIDER_UPLOADS_ROOT, path.join(folderPath, 'uploads', 'providers-data'));
      uploadsBytes += copyDirRecursive(REQUEST_UPLOADS_ROOT, path.join(folderPath, 'uploads', 'requests'));
      uploadsBytes += copyDirRecursive(DATA_REQUEST_UPLOADS_ROOT, path.join(folderPath, 'uploads', 'requests-data'));
    }
    ...
    console.warn('[Fandez Backup] Backup sin copia en disco:', err.message);
  ...
  let github = null;
  if (backupGithub.isGithubBackupEnabled()) {
    try {
      github = await backupGithub.pushBackupToGithub({ manifest, snapshot });
      if (github?.success) {
        console.log(`[Fandez Backup] GitHub OK → ${github.repo}:${github.path} (${github.bytes} bytes)`);
      } else if (github?.error) {
        console.warn('[Fandez Backup] GitHub:', github.error);
      }
    } catch (err) {
      console.error('[Fandez Backup] GitHub push falló:', err.message);
      github = { error: err.message };
    }
  }

  return { manifest, folderPath, folderName, snapshot, github };
}
...
async function listBackups() {
  ...
  if (backupStore.isAvailable()) {
    try {
      ...
      console.error('[Fandez Backup] listBackups MySQL:', err.message);
    }
  }
...
async function getBackupById(id) {
  if (backupStore.isAvailable()) {
    try {
      ...
      console.error('[Fandez Backup] getBackupById MySQL:', err.message);
    }
  }
...
async function deleteBackup(id) {
  ...
  if (backupStore.isAvailable()) {
    try {
      ...
      console.error('[Fandez Backup] deleteBackup MySQL:', err.message);
    }
  }
  return true;
}
...
async function readSnapshot(backupId) {
  if (backupStore.isAvailable()) {
    try {
      ...
      console.error('[Fandez Backup] readSnapshot MySQL:', err.message);
    }
  }
...
function restoreUploadsFromBackup(backup) {
  ...
  const requestsDataSrc = path.join(srcRoot, 'requests-data');
  if (fs.existsSync(requestsDataSrc)) {
    if (fs.existsSync(DATA_REQUEST_UPLOADS_ROOT)) {
      fs.rmSync(DATA_REQUEST_UPLOADS_ROOT, { recursive: true, force: true });
    }
    copyDirRecursive(requestsDataSrc, DATA_REQUEST_UPLOADS_ROOT);
  }
  const providersDataSrc = path.join(srcRoot, 'providers-data');
  if (fs.existsSync(providersDataSrc)) {
    if (fs.existsSync(DATA_PROVIDER_UPLOADS_ROOT)) {
      fs.rmSync(DATA_PROVIDER_UPLOADS_ROOT, { recursive: true, force: true });
    }
    copyDirRecursive(providersDataSrc, DATA_PROVIDER_UPLOADS_ROOT);
  } else if (fs.existsSync(UPLOADS_ROOT)) {
    // Compat: copiar public → data si el backup no traía providers-data
    copyDirRecursive(UPLOADS_ROOT, DATA_PROVIDER_UPLOADS_ROOT);
  }
  return true;
}
...
async function runScheduledBackups(store) {
  ...
  return { results, removed, config: updated, github: results[0]?.github || null };
}

async function runGithubBackupSync(store, { type = 'daily', triggeredBy = 'github-action' } = {}) {
  const result = await createBackup(store, type, triggeredBy);
  if (!backupGithub.isGithubBackupEnabled()) {
    return {
      success: true,
      backup: result.manifest,
      github: result.github || { skipped: true, reason: 'push opcional no configurado; usa export-encrypted desde Actions' }
    };
  }
  if (result.github?.error) {
    return { success: false, backup: result.manifest, github: result.github, error: result.github.error };
  }
  return { success: true, backup: result.manifest, github: result.github };
}

async function exportEncryptedBackup(store, { type = 'daily', triggeredBy = 'github-action' } = {}) {
  if (!backupGithub.hasEncryptKey()) {
    return { error: 'Falta BACKUP_GITHUB_ENCRYPT_KEY en Hostinger.' };
  }
  const result = await createBackup(store, type, triggeredBy);
  const packed = backupGithub.encryptSnapshotPackage({
    manifest: result.manifest,
    snapshot: result.snapshot
  });
  if (packed.error) return { error: packed.error, backup: result.manifest };
  return {
    success: true,
    backup: result.manifest,
    package: packed
...
function startBackupScheduler(store, logEvent) {
  ...
  if (!config.nextBackupAt) {
    saveConfig({ nextBackupAt: computeNextBackupAt(config) }).catch((err) => {
      console.error('[Fandez Backup] Error guardando nextBackupAt:', err.message);
    });
  }
  ...
  setInterval(async () => {
    ...
    try {
      ...
      console.log(`[Fandez Backup] Completado: ${result.results.length} copia(s)${result.removed ? `, ${result.removed} antigua(s) eliminada(s)` : ', historial conservado'}`);
    } catch (err) {
      await saveConfig({ lastBackupStatus: 'error', lastBackupError: err.message });
      console.error('[Fandez Backup] Error:', err.message);
    }
  }, 60 * 1000);
}
...
async function ensureStartupBackup(store) {
  ...
  try {
    ...
    if (backups.length === 0 || hoursSinceLast >= 6) {
      ...
      console.log('[Fandez Backup] Copia de arranque guardada en MySQL');
    }
  } catch (err) {
    console.error('[Fandez Backup] Copia de arranque fallida:', err.message);
  }
}
...
function getRetentionSummary(config) {
  ...
  return {
    ...
    marketReference: {
      daily: '7–30 días (Fandez: 14 por defecto)',
      weekly: '4–12 semanas (Fandez: 8 por defecto)',
      monthly: '12–36 meses (Fandez: 12 por defecto)',
...
module.exports = {
  ...
  formatBytes,
  BACKUPS_ROOT,
  githubStatus: () => backupGithub.getStatus()
};
