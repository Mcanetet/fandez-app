...
function normalizePayTerms(raw) {
  ...
  const hideFromTechnician = Boolean(
    raw.hideFromTechnician
    || raw.hide_from_technician
    || raw.payHideFromTechnician
    || raw.pay_hide_from_technician
  );
...
/**
 * Vista del técnico: si el socio ocultó el pago, no exponer montos ni %.
 */
function redactPaySnapshotForTechnician(snap) {
  if (!snap || snap.selfOperator || !snap.hideFromTechnician) return snap;
  return {
    selfOperator: false,
    configured: true,
    hideFromTechnician: true,
    hidden: true,
    mode: null,
    value: null,
    label: 'Tu pago lo coordina tu socio (no se muestra en la app)',
    providerPayout: null,
    technicianPayout: null,
    providerKeep: null,
...
module.exports = {
  ...
  redactPaySnapshotForTechnician
};
