/**
 * Modo de operación Fandez: demo (pruebas) vs production (pagos reales).
 *
...
/** URL absoluta del admin (fuerza APP_URL / www para no caer en apex roto). */
function absoluteAdminUrl(suffix = '') {
  const path = adminUrl(suffix);
...
function assertSecureBoot() {
  ...
  if (isProductionMode()) {
    ...
    if (mp.isConfigured() && !process.env.MP_WEBHOOK_SECRET) {
      errors.push('MP_WEBHOOK_SECRET es obligatorio en producción con Mercado Pago (validación de webhook).');
    }
  }
  return errors;
}
