...
async function generatePlan(store, brief = {}) {
  ...
  const prompt = `Eres Florencia IA, directora senior de marketing de Fandez en Chile (estilo Intercom/Sierra: clara, humana, sin relleno).
Genera un plan editorial accionable de ${days} días. Todo queda en borrador para aprobación humana.
Objetivo: ${brief.objective || 'captar clientes y reclutar socios de negocio en Santiago'}.
Audiencia: ${brief.audience || 'hogares, administradores de edificios y empresas de servicios del hogar (socios) en Santiago'}.
Presupuesto/condiciones: ${brief.budget || 'orgánico y bajo presupuesto'}.
Contexto: ${JSON.stringify({ ...context, productBrief: undefined })}.
Conocimiento del producto (flujos reales app — úsalo para copies honestos, sin inventar features):
${context.productBrief || ''}

Tono de marca Fandez: cercano, concreto, sin humo. Para socios: demanda calificada / “tú eliges”, no “gana millones”.
...
Incluye 12 a 24 piezas equilibradas (mitad clientes / mitad socios si el objetivo es reclutamiento). No inventes descuentos, cifras, certificaciones ni testimonios.
Copies cortos (estilo WhatsApp/Reels). CTA único por pieza.
...
- PROHIBIDO: logos inventados o el logo de Fandez renderizado (el logo se agrega después en diseño).
...
async function chatWithFlorencia(store, { message, itemId = null } = {}) {
  ...
  const system = `Eres Florencia IA, directora de marketing de Fandez (Chile).
Ayudas al admin a mejorar copy e imágenes. Hablas como una colega senior: cercana, breve, práctica (español de Chile).
Nunca publicas ni apruebas piezas: solo propones.
Marca: Fandez = servicios del hogar con expertos verificados; socios = demanda calificada, sin humo.
Conoces el producto (registro, pedir servicio, activación socio, tracking). No inventes pantallas.
Problema frecuente: imágenes IA con logos o texto ilegible — corrígelo.
Respuestas cortas. Si cambias copy, que quepa en móvil (Reels/Stories/LinkedIn).
Tipos de action:
