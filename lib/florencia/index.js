...
const { buildCompactSystemBrief } = require('../agents/systemKnowledge');
const { BRAND, FLORENCIA_PERSONA, FLORENCIA_IMAGE_RULES } = require('./brand');
const { stampOfficialLogo } = require('./composeCreative');
...
function marketingContext(store) {
  ...
  const productBrief = buildCompactSystemBrief(store, company).slice(0, 12000);
  return {
...
async function generatePlan(store, brief = {}) {
  ...
  const prompt = `${FLORENCIA_PERSONA}

Genera un plan editorial accionable de ${days} días para iniciar/impulsar la campaña de reclutamiento de socios en Instagram y otras redes. Todo queda en borrador para aprobación humana.
Objetivo: ${brief.objective || 'reclutar socios (empresas y profesionales) en Santiago + captar clientes'}.
Audiencia: ${brief.audience || 'empresas de gasfitería/electricidad/cerrajería y hogares en Santiago'}.
Presupuesto/condiciones: ${brief.budget || 'orgánico y bajo presupuesto'}.
Contexto numérico: ${JSON.stringify({ ...context, productBrief: undefined })}.
Producto (flujos reales):
${context.productBrief || ''}

Identidad visual obligatoria:
- Logo oficial: isotipo contorno ámbar #C45C14 (destapador/infinito) + wordmark Fandez.
- PROHIBIDO inventar logos (p.ej. dos cabezas).
- CTA socios: “${BRAND.ctaPartner}” → ${BRAND.linkPartner}
- Hook madre: “${BRAND.taglinePartner}”

Devuelve SOLO JSON válido con:
{
  "strategy": {"positioning":"","objectives":[],"audiences":[],"pillars":[],"kpis":[],"visualSystem":""},
  "items": [
    {
      "title":"",
...
Incluye 12 a 24 piezas (prioriza reclutamiento socios en IG/Stories/Reels/LinkedIn). Copies cortos. CTA único.
En imagePrompt: SOLO escena fotográfica (sin logos ni texto; el logo oficial se aplica en composición).`;

  const completion = await openai.chatCompletion({
    model: process.env.FLORENCIA_MODEL || process.env.OPENAI_MODEL || 'gpt-4o-mini',
    messages: [
      { role: 'system', content: `${FLORENCIA_PERSONA}\nNo publicas sin aprobación. Fotos IA sin logos ni texto.` },
...
async function generateImage(item, { extraInstructions = '' } = {}) {
  ...
  const prompt = `Fotografía publicitaria premium para Fandez (servicios técnicos del hogar en Santiago de Chile).
Diseñada bajo dirección de Florencia (gerente de marketing + diseñadora senior 20+ años + UX/UI).
Escena: ${imagePrompt}
${extraInstructions ? `Notas de dirección creativa: ${extraInstructions}` : ''}
...
${FLORENCIA_IMAGE_RULES}`;
  ...
  const rawTarget = path.join(IMAGE_DIR, `raw-${filename}`);
  if (b64) {
    await fs.promises.writeFile(rawTarget, Buffer.from(b64, 'base64'));
  } else if (remoteUrl) {
    const imageResponse = await fetch(remoteUrl);
    if (!imageResponse.ok) throw new Error('No se pudo descargar la imagen generada');
    await fs.promises.writeFile(rawTarget, Buffer.from(await imageResponse.arrayBuffer()));
  } else {
    throw new Error('OpenAI no devolvió una imagen');
  }

  // Estampa isotipo + wordmark oficiales (nunca logos inventados por la IA)
  try {
    await stampOfficialLogo(rawTarget, target, { corner: 'top-left', variant: 'light', markSize: 88 });
    await fs.promises.unlink(rawTarget).catch(() => {});
  } catch (err) {
    console.error('[florencia] stamp logo:', err.message);
    await fs.promises.rename(rawTarget, target).catch(async () => {
      await fs.promises.copyFile(rawTarget, target);
    });
  }
  ...
  return updateItem(item.id, {
    ...
    content: {
      ...
      brandLogo: 'fandez-01-contorno-oficial'
    }
  });
}
...
async function sendEmailCampaign(item, store) {
  ...
  const image = item.imageUrl
    ? `<img src="${publicImageUrl(item.imageUrl)}" alt="" width="504" style="width:100%;max-width:504px;height:auto;border:0;border-radius:14px;display:block;margin:0 0 18px">`
    : '';
  let sent = 0;
  let failed = 0;
  for (const user of recipients) {
    const result = await mailer.sendMail({
      ...
      html: transactional({
        title: subject,
        preheader: copy.slice(0, 90),
        eyebrow: 'Fandez',
        heading: subject,
        greeting: `Hola ${user.name || 'cliente'},`,
        bodyHtml: `${image}<p style="margin:0 0 14px;font-size:15px;line-height:1.55;color:#1A1814">${escapeHtml(copy).replace(/\n/g, '<br>')}</p>${cta ? `<p style="margin:0 0 8px;font-size:15px;font-weight:700;color:#1A1814">${escapeHtml(cta)}</p>` : ''}`,
        cta: cta && /^https?:\/\//i.test(cta) ? { href: cta, label: 'Ver más' } : { href: company.appUrl, label: 'Abrir Fandez' },
        note: 'Recibes este correo porque aceptaste comunicaciones de marketing de Fandez. Puedes darte de baja respondiendo “unsubscribe”.',
        supportHint: true
      })
    });
...
async function chatWithFlorencia(store, { message, itemId = null } = {}) {
  ...
  const system = `${FLORENCIA_PERSONA}

Ayudas al admin a mejorar copy e imágenes para la campaña de socios y el resto del plan.
Nunca publicas ni apruebas piezas: solo propones.
Logo: SOLO isotipo oficial contorno #C45C14 (nunca “dos cabezas”).
Si la imagen tiene logo inventado: pide regenerar foto limpia (sin logos) para estampar el oficial.
Respuestas cortas. Copies para móvil.
Tipos de action:
...
Si piden regenerar o arreglar logo/texto: update_image_prompt + regenerate_image.
imagePrompt y extraInstructions NUNCA piden logos, tipografía ni texto sobre la imagen.
Sin pieza vinculada: consejo estratégico, actions [].
...
module.exports = {
  ...
  marketingRecipients,
  startScheduler,
  generatePartnerCampaignSet: (...args) => require('./composeCreative').generatePartnerCampaignSet(...args),
