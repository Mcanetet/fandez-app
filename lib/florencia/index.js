...
async function sendEmailCampaign(item, store) {
  ...
  for (const user of recipients) {
    const result = await mailer.sendMail({
      ...
      html: `${image}<p>Hola <strong>${escapeHtml(user.name || 'cliente')}</strong>,</p><p>${escapeHtml(copy).replace(/\n/g, '<br>')}</p><p><strong>${escapeHtml(cta)}</strong></p><p style="font-size:11px;color:#6B7280">Recibes este correo porque aceptaste comunicaciones de marketing de Fandez. Puedes darte de baja respondiendo “unsubscribe”.</p>`
    });
