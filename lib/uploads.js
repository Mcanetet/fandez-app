...
const DATA_PROVIDER_ROOT = path.resolve(__dirname, '../data/uploads/providers');
...
const MAX_BYTES = Math.min(
  8 * 1024 * 1024,
  Math.max(100 * 1024, Number(process.env.UPLOAD_MAX_BYTES) || 5 * 1024 * 1024)
);
...
function sniffMime(buf) {
  ...
  if (isHeicBuffer(buf)) return 'image/heic';
  return null;
}
...
function parseProviderDocUrl(stored) {
  const url = String(stored || '').trim().replace(/\\/g, '/');
  if (!url || url.startsWith('data:') || url === 'demo') return null;
  const match = url.match(/^\/(?:uploads\/providers|media\/provider)\/([^/]+)\/([^/?#]+)/);
  if (!match) return null;
  return { providerId: safeId(match[1]), file: safeFileName(match[2]) };
}
...
function toServingUrl(stored) {
  ...
  const providerParsed = parseProviderDocUrl(value);
  if (providerParsed && providerParsed.providerId && providerParsed.file) {
    return `/media/provider/${providerParsed.providerId}/${providerParsed.file}`;
  }
  if (value.startsWith('/uploads/') || value.startsWith('/media/')) return value.replace(/\\/g, '/');
  const asPublic = toPosixUrl(value, PUBLIC_ROOT, '');
  if (asPublic) {
    const normalized = asPublic.startsWith('/') ? asPublic : `/${asPublic}`;
    const mappedReq = parseRequestPhotoUrl(normalized);
    if (mappedReq) return `/media/request/${mappedReq.requestId}/${mappedReq.file}`;
    const mappedProv = parseProviderDocUrl(normalized);
    if (mappedProv) return `/media/provider/${mappedProv.providerId}/${mappedProv.file}`;
    return normalized;
  }
  const asDataReq = toPosixUrl(value, DATA_REQUEST_ROOT, '/uploads/requests');
  if (asDataReq) {
    const mapped = parseRequestPhotoUrl(asDataReq);
    if (mapped) return `/media/request/${mapped.requestId}/${mapped.file}`;
    return asDataReq;
  }
  const asDataProv = toPosixUrl(value, DATA_PROVIDER_ROOT, '/uploads/providers');
  if (asDataProv) {
    const mapped = parseProviderDocUrl(asDataProv);
    if (mapped) return `/media/provider/${mapped.providerId}/${mapped.file}`;
    return asDataProv;
  }
...
function copyToProviderDataDir(absPublicPath, providerId) {
  try {
    const destDir = path.join(DATA_PROVIDER_ROOT, safeId(providerId));
    fs.mkdirSync(destDir, { recursive: true });
    const dest = path.join(destDir, path.basename(absPublicPath));
    if (path.resolve(absPublicPath) !== path.resolve(dest)) {
      fs.copyFileSync(absPublicPath, dest);
...
function findNewestProviderFile(providerId, fileHint) {
  const id = safeId(providerId);
  if (!id) return null;
  const hint = String(fileHint || '');
  const prefix = hint.split('-')[0] || '';
  const dirs = [
    path.join(DATA_PROVIDER_ROOT, id),
    path.join(UPLOAD_ROOT, id)
  ];
  let best = null;
  let bestMtime = 0;
  for (const dir of dirs) {
    try {
      ...
      for (const name of fs.readdirSync(dir)) {
        ...
        if (!/\.(jpe?g|png|webp|pdf)$/i.test(name)) continue;
...
function resolveProviderDocPath(stored) {
  const parsed = parseProviderDocUrl(stored);
  if (!parsed || !parsed.providerId || !parsed.file) {
    if (stored && fs.existsSync(stored)) return path.resolve(stored);
    return null;
  }
  const candidates = [
    path.join(DATA_PROVIDER_ROOT, parsed.providerId, parsed.file),
    path.join(UPLOAD_ROOT, parsed.providerId, parsed.file)
  ];
  for (const candidate of candidates) {
    const resolved = path.resolve(candidate);
    if (fs.existsSync(resolved)) return resolved;
  }
  // Redeploy / rename: buscar el más reciente con el mismo prefijo (idFront, selfie, …)
  return findNewestProviderFile(parsed.providerId, parsed.file);
}
...
function saveBase64File(dir, filename, dataUrlOrBase64) {
  ...
  if (buf.length > MAX_BYTES) {
    throw new Error(`El archivo supera ${Math.round(MAX_BYTES / (1024 * 1024))} MB. Toma la foto de nuevo o elige un JPEG más liviano.`);
  }
  const sniffed = sniffMime(buf);
  if (sniffed === 'image/heic' || claimedMime === 'image/heic' || claimedMime === 'image/heif') {
    throw new Error('La foto del celular llegó en HEIC. Tómala de nuevo o elige un JPEG desde Archivos; la app la comprime sola al subir.');
  }
  const mime = sniffed || (ALLOWED[claimedMime] ? claimedMime : null);
  if (!mime || !ALLOWED[mime]) {
    throw new Error('Tipo de archivo no permitido (solo JPG, PNG, WEBP o PDF). Si tomaste la foto con el celular, espera a que se guarde e inténtalo otra vez.');
  }
...
function saveProviderFile(providerId, category, dataUrlOrBase64) {
  ...
  copyToProviderDataDir(fullPath, id);
  return `/uploads/providers/${id}/${path.basename(fullPath)}`;
}

function stableProviderDocUrl(providerId, fileOrKind) {
  const id = safeId(providerId);
  const file = safeFileName(fileOrKind);
  if (!id || !file) return null;
  return `/media/provider/${id}/${file}`;
}
...
function stableRequestPhotoUrl(requestId, kind) {
  const id = safeId(requestId);
  if (!id) return null;
  const k = kind === 'brand' ? 'brand' : 'problem';
  return `/media/request/${id}/kind/${k}`;
}

function stableTechnicianPhotoUrl(technicianId) {
  const id = safeId(technicianId);
  return id ? `/media/technician/${id}/photo` : null;
}

function resolveTechnicianPhotoPath(stored) {
  if (!stored || stored === 'demo') return null;
  const raw = String(stored).trim();
  if (!raw) return null;
  // Ruta absoluta en data/technician-documents
  if (path.isAbsolute(raw) && fs.existsSync(raw)) return raw;
  // Selfie / carnet de socio
  const providerAbs = resolveProviderDocPath(raw);
  if (providerAbs) return providerAbs;
  if (raw.startsWith('/uploads/')) {
    const abs = path.join(PUBLIC_ROOT, raw.replace(/^\//, ''));
    if (fs.existsSync(abs)) return abs;
  }
  // Relativo a technician-documents
  const underPrivate = path.join(PRIVATE_TECHNICIAN_ROOT, raw.replace(/^\/+/, ''));
  if (fs.existsSync(underPrivate)) return underPrivate;
  return null;
}
