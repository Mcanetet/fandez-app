...
const { buildSystemKnowledgeBlocks } = require('../agents/systemKnowledge');
...
async function syncKnowledgeFromApp(appStore) {
  ...
  const companyBlock = [
    ...
    `Teléfono: ${company.whatsappDisplay}`,
    `Web: ${company.appUrl}`,
  ...
  await alandStore.saveKnowledge({
    id: 'kb-company',
    sourceType: 'company',
    title: 'Información de la empresa Fandez',
  ...
  const pricingBlock = [
    ...
    `Opciones de llegada: ${(pricing.urgencyTiers || []).map((t) => {
      const pct = Number(t.surchargePercent != null ? t.surchargePercent : t.adjustmentPercent) || 0;
      const adj = pct > 0 ? `+${pct}%` : pct < 0 ? `${pct}%` : 'precio normal';
      return `${t.label} (${adj})`;
    }).join(', ')}`,
  ...
  await alandStore.saveKnowledge({
    id: 'kb-pricing',
    sourceType: 'pricing',
    title: 'Precios y tarifas Fandez',
  ...
  const catalogBlock = [
    `Fandez ofrece ${enabledServices.length} servicios a domicilio en Santiago:`,
    ...enabledServices.map((s, i) => `${i + 1}. ${s.name} — visita desde ${formatCLP(s.visitPrice)}. ${s.description || ''}`),
    '',
    'Sitio público: https://www.fandez.cl (no uses rutas de administración).',
  ...
  await alandStore.saveKnowledge({
    id: 'kb-catalog',
    sourceType: 'catalog',
    title: 'Catálogo de servicios Fandez',
  ...
  const attentionBlock = [
    'Checklist rápido de atención Fandez — socio y técnico:',
    '1. Llegar a tiempo, presentarse y pedir permiso para entrar.',
    '2. Explicar el diagnóstico y pedir OK en la app antes de cambios o costos extra.',
    '3. Fotos de inicio/cierre y dejar el área ordenada.',
    '4. Despedirse y recordar calificar en la app.',
    'Sofía acompaña al cliente en cada hito. Soporte operativo: Sofía en la app.'
  ].join('\n');
  ...
  // Mapa completo del producto: pantallas, botones y flujos (clientes/socios/técnicos)
  for (const block of buildSystemKnowledgeBlocks(appStore, company)) {
    await alandStore.saveKnowledge({
      id: block.id,
      sourceType: block.sourceType || 'system',
      title: block.title,
...
function shouldEscalate(text, config) {
  ...
  if (routing.looksLikePersonalSafetyTopic?.(raw)) {
    return { escalate: true, reason: 'personal_safety', escalateAdmin: true };
  }
  if (routing.looksLikeSafetyEmergency(raw)) {
    return { escalate: true, reason: 'safety' };
  }
  const keywords = [...(config.escalateKeywords || [])];
  if (keywords.some((kw) => lower.includes(String(kw).toLowerCase()))) {
    return { escalate: true, reason: 'keyword' };
  }

  // Solo patrones fuertes (no “no funciona” genérico)
  const hardPatterns = [
    /filtraci[oó]n\s+(grave|grande|en\s+el\s+techo)/i,
    /presupuesto\s+en\s+terreno/i,
    /doble\s+cobro/i,
    /reembolso|devoluci[oó]n\s+de\s+dinero/i
  ];
  if (hardPatterns.some((re) => re.test(raw))) return { escalate: true, reason: 'technical' };
...
function buildSystemPrompt(config, knowledgeContext, serviceName, liveContext = null) {
  return [
    config.personality,
    '',
    `Tu nombre es ${config.agentName || 'Sofía'}. Nunca digas que eres ChatGPT ni un bot genérico.`,
    ...
    'Si el tema es de pagos/cobros/facturación, responde con el estado conocido y termina con [DERIVAR_PAGOS] en una línea aparte.',
    '',
...
async function generateAlandReply({ appStore, conversation, userMessage, config, user = null }) {
  ...
  try {
    ...
    if (enabled.length) {
      const liveCatalog = [
        `### Catálogo en vivo Fandez (${enabled.length} servicios)`,
  ...
  try {
    const sofiaTools = require('./sofiaTools');
    const clientUser = user || (conversation.clientId
      ? { id: conversation.clientId, role: 'client' }
      : null);
    if (clientUser) {
      if (!appStore.getClientRequests && typeof appStore.getRequestsByClient === 'function') {
        appStore.getClientRequests = (id) => appStore.getRequestsByClient(id);
      }
      liveContext = sofiaTools.buildClientLiveContext(appStore, clientUser);
    }
  } catch (_) { /* tools opcionales */ }

  const history = await alandStore.listMessages(conversation.id, 30);
  const messages = [
    {
      role: 'system',
      content: buildSystemPrompt(config, knowledgeContext, conversation.serviceName, liveContext)
    }
  ];
  ...
  if (escalatePayments && !/pago|cobro|admin|equipo|factur|deriv/i.test(reply)) {
    reply += '\n\nDerivé tu consulta al equipo de pagos de Fandez. La revisarán en la plataforma; puedes seguir escribiendo aquí si agregas detalle.';
  } else if (escalate && !/especialista|equipo|deriv/i.test(reply)) {
    reply += '\n\nDerivé tu caso al equipo del servicio. Si no responden a tiempo, administración Fandez lo retoma. Puedes seguir escribiendo aquí.';
  }
...
async function escalateToAdmin(conversation, io, reason = 'timeout') {
  ...
  const bodyByReason = {
    timeout: 'El socio no respondió a tiempo. Conversación escalada a administración Fandez.',
    ...
    provider_signup: 'Nueva inscripción de socio. Sofía dejó los datos para revisión (empresa, contacto y servicios solicitados).',
    safety: 'Posible urgencia de seguridad del hogar. Revisar de inmediato y coordinar con el socio / emergencias si aplica.'
  };
  ...
  const message = await alandStore.addMessage({
    ...
    senderType: 'system',
    senderName: 'Sistema',
    body: bodyByReason[reason] || 'Conversación escalada a administración Fandez.',
...
async function escalatePaymentIssue({ appStore, conversation, io, user, userMessage }) {
  ...
  const handoff = await alandStore.addMessage({
    ...
    senderType: 'aland',
    senderName: 'Sofía',
    body: 'Derivé tu consulta al equipo de Fandez. La revisarán en la plataforma. Mientras tanto puedes agregar detalle aquí si lo necesitas.',
...
async function processClientMessage({ appStore, io, conversationId, user, text }) {
  ...
  if (!config.enabled) {
    throw new Error('Sofía está desactivada temporalmente');
  }
  ...
  // Seguridad interpersonal: script fijo + alerta admin (no improvisar con LLM)
  if (routing.looksLikePersonalSafetyTopic?.(text)) {
    const safetyLib = require('../safety');
    const reply = safetyLib.buildSafetyReplyEs();
    const alandMsg = await alandStore.addMessage({
      conversationId,
      senderType: 'aland',
      senderName: config.agentName || 'Sofía',
      body: reply,
      meta: { model: 'safety_script', reason: 'personal_safety', securityAlert: true }
    });
    if (io) {
    ...
    conversation = await escalateToAdmin(conversation, io, 'personal_safety');
    try {
      require('../agents/founderAlerts').notifyInstant({
        agent: 'Sofía',
        severity: 'critical',
        title: 'Chat · posible amenaza a integridad',
        body: [
          `Cliente: ${conversation.clientName || user?.email || '—'}`,
          `Conversación: ${conversationId}`,
          `Texto: ${String(text).slice(0, 400)}`,
          'Acción: 133 si hay riesgo + revisar SOS en la visita.'
        ].join('\n'),
        dedupeKey: `sofia-safety-${conversationId}-${Date.now() - (Date.now() % 600000)}`
      }).catch(() => {});
    } catch (_) { /* ignore */ }
    try {
      if (typeof appStore.logSecurityEvent === 'function') {
        appStore.logSecurityEvent(
          'aland_personal_safety',
    ...
    return {
      ...
      escalated: true,
      escalateReason: 'personal_safety'
    };
  }
