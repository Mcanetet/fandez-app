/**
 * Avisos gratis al admin (sin canales externos).
 * Canales:
...
function alertEmails() {
  const founderRaw = [process.env.FOUNDER_EMAILS, process.env.FOUNDER_EMAIL]
    .filter(Boolean)
    .join(',');
  const founders = founderRaw
    .split(/[,;\s]+/)
