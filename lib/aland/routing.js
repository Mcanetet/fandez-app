 ...
 * Sin canales externos de mensajería hacia clientes.
 */
...
function classifyTopic(text) {
  ...
  try {
    const { looksLikePersonalSafety } = require('../safety');
    if (looksLikePersonalSafety(raw)) return 'personal_safety';
  } catch (_) { /* optional */ }
...
function looksLikePersonalSafetyTopic(text) {
  try {
    return require('../safety').looksLikePersonalSafety(text);
  } catch (_) {
    return false;
  }
}
