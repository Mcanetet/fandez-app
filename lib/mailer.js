...
function formatFromAddress() {
  ...
  if (!email) return `"Fandez" <${company.supportEmail}>`;
  return `"Fandez" <${email}>`;
}
...
function wrapHtmlDocument(bodyHtml, { title = 'Fandez', preheader = '' } = {}) {
  return emailLayout.wrapHtmlDocument(bodyHtml, { title, preheader });
}
...
async function sendMail({ to, subject, text, html, replyTo }) {
  ...
  try {
    ...
    const plainText = (text && String(text).trim())
      ...
      || subject;
    const htmlBody = html
      ? wrapHtmlDocument(html, { title: subject || 'Fandez', preheader: subject || '' })
      : wrapHtmlDocument(emailLayout.plainTextToBody(plainText), {
        title: subject || 'Fandez',
        preheader: subject || ''
      });

    const domain = (fromEmail && fromEmail.includes('@')) ? fromEmail.split('@')[1] : 'fandez.cl';
    ...
    const info = await getTransporter().sendMail({
      ...
      messageId,
      // Evitar X-Priority:1 — Outlook/Hotmail lo trata como spam con frecuencia.
      headers: {
        'X-Mailer': 'Fandez',
        'X-Auto-Response-Suppress': 'OOF, AutoReply',
        'Auto-Submitted': 'auto-generated',
        'X-Entity-Ref-ID': `fandez-${Date.now()}`,
