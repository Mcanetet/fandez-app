...
function assertRutAvailableForAccount(rut, { users, excludeUserId, purpose } = {}) {
  ...
  const conflict = list.find((u) => {
    ...
    // Facturación cliente: solo choca con otras cuentas role=client.
    // Los socios (role=provider) pueden compartir RUT al pedir servicios.
    if (purpose === 'client_billing' && u.role !== 'client') return false;
    // Identidad socio: solo choca con otras cuentas socio.
    if (purpose === 'provider_identity' && u.role !== 'provider') return false;
