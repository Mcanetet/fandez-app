...
module.exports = {
  ...
  onTechnicianAssigned(request) {
    ...
    // Push + socket al celular del técnico (aunque la app esté cerrada)
    if (request.technicianId && !technician?.isSelfOperator) {
      try {
        ...
        const body = `${service} · Tienes ${mins} min para aceptar`;
        const url = `/tecnico?pedido=${encodeURIComponent(request.id)}`;
        webPush.notifyUsers([request.technicianId], {
          title: 'Fandez — Te asignaron un pedido',
          body,
          url,
          tag: `fandez-tec-assign-${request.id}`,
          requireInteraction: true
