...
function getProviderOnboardingSteps({ hasVerificationBanner = false, t } = {}) {
  ...
  steps.push(
    ...
    {
      target: '[data-tour="work-wall"]',
      title: tr('provider.onboarding.wall_title'),
      body: tr('provider.onboarding.wall_body')
    },
    {
...
function providerTechnicalCoverageOk(provider, store) {
  const specs = Array.isArray(provider?.specialties) ? provider.specialties : [];
  if (!specs.length || !store) return false;
  return specs.every((sid) => store.hasTechnicianCoverage(provider.id, sid));
}

function getProviderActivationSteps(provider, verificationCheck, contractSummary, storeRef) {
  const specialtiesOk = Array.isArray(provider?.specialties) && provider.specialties.length > 0;
  const coverageOk = providerTechnicalCoverageOk(provider, storeRef);
  ...
  const steps = [
    ...
    },
    {
      ...
      labelKey: 'provider.activation.coverage',
      url: '/proveedor/equipo'
    },
    {
      id: 'online',
  ...
  if (coverageOk && provider?.online && taken === 0) {
    steps.push({
      id: 'first_job',
      done: false,
      labelKey: 'provider.activation.first_job',
      url: null,
      highlight: true
    });
  }
...
const ATTENTION_CHECKLIST = [
  { id: 'arrive_present', label: 'Llegué a tiempo, me presenté y pedí permiso para entrar' },
  { id: 'explain_ok', label: 'Expliqué el diagnóstico y pedí OK antes de cambios o costos extra' },
  { id: 'photos_clean', label: 'Tomé fotos (inicio/cierre) y dejé el área ordenada' },
  { id: 'close_rate', label: 'Me despedí y recordé calificar en la app' }
];
