...
function sanitizeRequestForWorker(request, pricing) {
  ...
  // Nunca incluir teléfono, email, billing/RUT, guardianToken, chat ni arrivalCode.
  return {
    id: request.id,
    ...
    coords: request.coords && request.coords.lat != null
      ? { lat: Number(request.coords.lat), lng: Number(request.coords.lng) }
      : null,
    notes: request.notes || '',
