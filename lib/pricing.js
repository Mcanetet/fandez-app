...
const DEFAULT_PRICING = {
  ...
  /** % del cobro de mano de obra retenido al socio por tarjeta + administración (no se suma al cliente). */
  merchantCardFeePercent: 5,
  /** IVA Chile sobre comisiones + fee tarjeta */
  ivaRate: 0.19,
  /** Legacy: el cliente ya no ve ni paga recargo; el 5% se descuenta al socio en liquidación. */
...
function normalizePricing(raw) {
  ...
  let merchantFee = parseInt(base.merchantCardFeePercent, 10);
  if (!Number.isFinite(merchantFee) || merchantFee < 0) {
    merchantFee = DEFAULT_PRICING.merchantCardFeePercent;
  } else if (merchantFee === 4) {
    // Migración: fee tarjeta/admin del socio 4% → 5%.
    merchantFee = 5;
  }
  base.merchantCardFeePercent = merchantFee;
 ...
 *
 * Visitas diferidas (mañana / en 2 días / programado): no aplican recargo de
 * “tarde/madrugada” de la hora actual — el servicio ocurre en otro día.
 */
function calculateVisitPricing(pricing, tierId, { horaSolicitud = new Date(), valorBase, timeZone } = {}) {
  ...
  const deferredVisit = ['tomorrow', 'two_days', 'scheduled'].includes(String(tier.id || ''));
  const tariff = calculateDynamicTariff({
    ...
    timeZone,
    forceHorarioBand: deferredVisit ? 'normal' : undefined
  });
...
function calculatePaymentSurcharge(pricing, visitSubtotal, paymentMethod) {
  ...
  // El cliente paga solo el precio del servicio. El 5% tarjeta/admin se descuenta al socio en liquidación.
  return {
    method: 'card',
    percent: 0,
    amount: 0,
...
function computeRequestFinancials(request, pricing) {
  ...
  // Los materiales son un reembolso íntegro al socio: no generan comisión Fandez.
