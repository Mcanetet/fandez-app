...
const { isGardenIntakeService } = require('./gardenIntake');
const {
...
const DEFAULT_PRICING = {
  ...
  catalogPrices: {},
  /** Referencias mercado RM por actividad: { [activityId]: { low, mid, high } } */
  catalogMarketRefs: {},
  ...
  cancellations: {
    beforeAccepted: 0,
    afterTechAccepted: 20000,
    enRouteOrOnSite: 20000
  },
  /** @deprecated Usar cancellations.enRouteOrOnSite — se migra en normalizePricing */
  cancellationFee: 20000,
  /**
   * Referidos (crédito CLP). El ON/OFF del producto es el módulo client_referidos en Admin → Módulos.
   * Aquí solo se configura el monto cuando el módulo está activo.
   */
  referrals: {
    creditCLP: 5000,
    referrerPoints: 200
  ...
  /**
   * Materiales por debajo de este monto (CLP) van incluidos en el servicio (no se cobran).
   * Desde este monto en adelante: aviso al cliente → OK → ir a comprar → subir factura.
   */
  materialsIncludedThresholdClp: 10000,
  /** Boletas ≤ este monto pueden auto-aprobarse (si IA aprueba con confianza) */
  materialsAutoApproveMaxClp: 15000,
  /** Sobre este monto (o igual) → pending_founder aunque la IA apruebe */
  ...
  bankTransfer: {
    ...
    holderName: 'Gestión Integral y Servicios Fandez SpA',
    holderRut: '78.494.038-1',
...
function normalizePricing(raw) {
  ...
  const refSrc = (raw && raw.referrals && typeof raw.referrals === 'object')
    ? raw.referrals
    : (base.referrals && typeof base.referrals === 'object' ? base.referrals : {});
  const creditCLP = parseInt(refSrc.creditCLP, 10);
  const referrerPoints = parseInt(refSrc.referrerPoints, 10);
  base.referrals = {
    creditCLP: Number.isFinite(creditCLP) && creditCLP >= 0
      ? creditCLP
      : DEFAULT_PRICING.referrals.creditCLP,
    referrerPoints: Number.isFinite(referrerPoints) && referrerPoints >= 0
      ? referrerPoints
      : DEFAULT_PRICING.referrals.referrerPoints
  };
  ...
  const matThreshold = parseInt(base.materialsIncludedThresholdClp, 10);
  base.materialsIncludedThresholdClp = Number.isFinite(matThreshold) && matThreshold >= 0
    ? matThreshold
    : DEFAULT_PRICING.materialsIncludedThresholdClp;
  const autoMax = parseInt(base.materialsAutoApproveMaxClp, 10);
  base.materialsAutoApproveMaxClp = Number.isFinite(autoMax) && autoMax >= 0
    ? autoMax
    : DEFAULT_PRICING.materialsAutoApproveMaxClp;
  const founderMin = parseInt(base.materialsFounderReviewMinClp, 10);
  base.materialsFounderReviewMinClp = Number.isFinite(founderMin) && founderMin >= 0
    ? founderMin
    : DEFAULT_PRICING.materialsFounderReviewMinClp;
  const autoConf = Number(base.materialsAutoApproveMinConfidence);
  base.materialsAutoApproveMinConfidence = Number.isFinite(autoConf) && autoConf > 0 && autoConf <= 1
    ? autoConf
    : DEFAULT_PRICING.materialsAutoApproveMinConfidence;
  ...
  try {
    const { normalizeCatalogMarketRefs } = require('./marketReferenceRm');
    base.catalogMarketRefs = normalizeCatalogMarketRefs(
      raw?.catalogMarketRefs != null ? raw.catalogMarketRefs : base.catalogMarketRefs
    );
  } catch (_) {
    base.catalogMarketRefs = base.catalogMarketRefs && typeof base.catalogMarketRefs === 'object'
      ? base.catalogMarketRefs
      : {};
  }
  ...
  base.materialsCatalog = mergeMaterialsCatalogWithDefaults(
...
function computeRequestFinancials(request, pricing) {
  ...
  const materialsTotal = (sr?.materials || []).reduce((s, m) => {
    if (m.reviewStatus === 'rejected') return s;
    if (m.includedInService === true || m.billable === false) return s;
    return s + (parseInt(m.amount, 10) || 0);
  }, 0);
...
function sanitizeRequestForWorker(request, pricing) {
  ...
  // Whitelist estricta: el muro llega a socios/técnicos ANTES de aceptar.
  // Sin calle completa ni coords precisas (solo comuna/zona). Sin PII ni arrivalCode.
  const zoneLabel = [request.communeName, request.regionName].filter(Boolean).join(', ')
    || 'Zona disponible';
  return {
    id: request.id,
    ...
    address: '',
    zoneLabel,
    communeName: request.communeName || null,
    ...
    beneficiaryName: request.isGift ? (request.beneficiaryName || null) : null,
    urgencyTier: request.urgencyTier || null,
    urgencyTierLabel: request.urgencyTierLabel || null,
    arrivalDisplay: request.arrivalDisplay || null,
...
function quoteActivityForRequest(pricing, activityId, { horaSolicitud, tierId, timeZone, squareMeters, serviceId, cleaningFactors } = {}) {
  const cfg = normalizePricing(pricing);
  ...
  const gardenIntake = found.activity?.quoteMode === 'garden_intake'
    || isGardenIntakeService(serviceId || found.specialtyId);
  let valorBase = perM2
    ? resolveM2QuoteBase(found.activity, squareMeters, { serviceId: serviceId || found.specialtyId })
    : found.activity.basePrice;
  if (cleaningFactors && (isCleaningService(serviceId) || String(activityId || '').startsWith('lim-'))) {
    valorBase = applyCleaningSurcharge(valorBase, cleaningFactors);
  }
  return calculateVisitPricing(cfg, tier.id, {
    horaSolicitud: horaSolicitud || new Date(),
    valorBase,
    timeZone,
    skipWorkFloor: perM2 || gardenIntake
  });
}
