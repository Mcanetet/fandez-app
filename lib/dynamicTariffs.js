/**
 * Motor de tarifas dinámicas Fandez (CLP).
 * Total = round((valorBase * recargoHorario) * recargoUrgencia)
 */

'use strict';

const { SERVICE_CATALOG, SERVICE_TO_SPECIALTY, isPerM2Activity } = require('./serviceCatalogData');
...
const MIN_WORK_BASE_CLP = 55000;
const MIN_DIAGNOSTIC_VISIT_CLP = 19990;
...
function getForcedScheduleBand(band, scheduleSurcharges = {}) {
  const normalPercent = Number.isFinite(Number(scheduleSurcharges.normalPercent))
    ? Number(scheduleSurcharges.normalPercent) : 0;
  const tardePercent = Number.isFinite(Number(scheduleSurcharges.tardePercent))
    ? Number(scheduleSurcharges.tardePercent) : 25;
  const nocturnoPercent = Number.isFinite(Number(scheduleSurcharges.nocturnoPercent))
    ? Number(scheduleSurcharges.nocturnoPercent) : 50;
  if (band === 'tarde') return { band: 'tarde', multiplier: 1 + tardePercent / 100, percent: tardePercent };
  if (band === 'nocturno') {
    return { band: 'nocturno', multiplier: 1 + nocturnoPercent / 100, percent: nocturnoPercent };
  }
  return { band: 'normal', multiplier: 1 + normalPercent / 100, percent: normalPercent };
...
function calculateDynamicTariff(input) {
  ...
  // 1) Base de trabajo (>= $55.000 salvo cobro por m²)
  const valorBaseOriginal = Number(valorBase);
  const valorBaseAplicado = normalizeWorkBase(valorBaseOriginal, { strictBase, skipWorkFloor });
  ...
  // Visitas diferidas (mañana / 2 días) no heredan el recargo de la hora actual.
  const schedule = forceHorarioBand
    ? getForcedScheduleBand(forceHorarioBand, scheduleSurcharges)
    : getScheduleMultiplier(time.totalMinutes, scheduleSurcharges);
...
function getServiceCatalog(priceOverrides = null) {
  ...
  return SERVICE_CATALOG.map((specialty) => ({
    ...
    activities: specialty.activities.map((activity) => {
      ...
      const m2 = isPerM2Activity(activity);
      const pricePerM2 = m2
        ? (Number.isFinite(override) && override > 0 ? override : (activity.pricePerM2 || activity.basePrice))
        : null;
      const basePrice = m2
        ? pricePerM2
        : (Number.isFinite(override) && override > 0
          ? (activity.quoteMode === 'garden_intake'
            ? Math.max(0, override)
            : Math.max(MIN_WORK_BASE_CLP, override))
          : activity.basePrice);
      return {
        ...activity,
        pricingUnit: m2 ? 'm2' : (activity.pricingUnit || 'job'),
        quoteMode: activity.quoteMode || null,
        pricePerM2,
        basePrice
      };
    })
  ...
  getServiceCatalog(priceOverrides).forEach((specialty) => {
    specialty.activities.forEach((activity) => {
      rows.push({
        ...
        kind: activity.kind,
        pricingUnit: activity.pricingUnit || 'job',
        pricePerM2: activity.pricePerM2 || null,
        minM2: activity.minM2 || null,
        quoteMode: activity.quoteMode || null,
...
function normalizeCatalogPrices(raw) {
  const byId = new Map(
    SERVICE_CATALOG.flatMap((s) => s.activities.map((a) => [a.id, a]))
  );
  const out = {};
  function accept(id, price) {
    const activity = byId.get(id);
    if (!activity || !Number.isFinite(price) || price <= 0) return;
    if (isPerM2Activity(activity)) {
      out[id] = Math.max(1000, Math.round(price));
    } else {
...
function calculateCatalogActivityTariff(activityId, opts) {
  ...
  return {
    ...
    tariff: calculateDynamicTariff({
      ...
      forceHorarioBand: opts.forceHorarioBand
    })
  };
}
