...
const TEMPLATES = {
  ...
  'payment.voucher': (ctx) => ({
    subject: `Comprobante de pago ${ctx.voucherCode || ''} — Fandez`.trim(),
    ...
    html: `
      ...
      <p><a href="${ctx.voucherUrl}" style="display:inline-block;background:#C45C14;color:#fff;text-decoration:none;padding:10px 16px;border-radius:10px;font-weight:600">Ver comprobante</a></p>
      <p style="color:#6B7280;font-size:13px;margin-top:20px">Este acuse de recibo no reemplaza los documentos tributarios que correspondan a Fandez y al socio prestador.</p>
    `
  }),
  'payment.transfer_pending': (ctx) => ({
    subject: `Transferencia pendiente — ${ctx.request.serviceName}`,
    text: `Hola ${ctx.client.name},\n\nRegistramos tu solicitud de transferencia por ${formatCLP(ctx.amount)}.\nReferencia: FANDEZ-${ctx.request.id.slice(0, 8).toUpperCase()}\n\nCuando confirmemos el abono, activaremos la búsqueda de técnico.`
  }),
  ...
  'service.no_provider': (ctx) => {
    ...
    html: `
      <p>Hola <strong>${ctx.client?.name || 'cliente'}</strong>,</p>
      <p>Han pasado <strong>${minutes} minutos</strong> y todavía no hemos encontrado un socio para tu servicio de <strong>${ctx.request.serviceName}</strong>.</p>
      <p><strong>¿Quieres que sigamos buscando?</strong></p>
      <p style="margin:20px 0">
        <a href="${ctx.continueUrl}" style="display:inline-block;background:#C45C14;color:#fff;text-decoration:none;padding:11px 16px;border-radius:10px;font-weight:600;margin:0 8px 8px 0">Sí, seguir buscando</a>
        <a href="${ctx.refundUrl}" style="display:inline-block;background:#C0392B;color:#fff;text-decoration:none;padding:11px 16px;border-radius:10px;font-weight:600">Solicitar devolución</a>
      </p>
      <p style="color:#6B7280;font-size:13px">También puedes elegir desde la app de Fandez.</p>
    `
    };
  },
  ...
  'technician.en_route': (ctx) => ({
    ...
    html: `
      <p>Hola <strong>${ctx.client.name}</strong>,</p>
      <p><strong>${ctx.request.technicianName || 'Tu técnico'}</strong> ya va en camino a tu domicilio.</p>
      <p style="color:#6B635A;font-size:14px">${ctx.request.address || ''}</p>
      <p style="margin:22px 0">
        <a href="${trackUrl(ctx.request)}" style="display:inline-block;background:#C45C14;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700">Ver en el mapa</a>
      </p>
      <p style="color:#6B635A;font-size:13px">También puedes seguirlo desde la app Fandez, como en Uber o PedidosYa.</p>
    `
  }),
  ...
  'service.job_voucher': (ctx) => ({
    subject: `Voucher final del trabajo ${ctx.voucherCode} — Fandez`,
  ...
  'dte.issued': (ctx) => {
    ...
    return {
      subject: `Tu ${kind} electrónica N° ${ctx.folio} — Fandez`,
      ...
      html: `
        ...
        <p><a href="${link}" style="display:inline-block;background:#C45C14;color:#fff;text-decoration:none;padding:10px 16px;border-radius:10px;font-weight:600">Descargar documento</a></p>
