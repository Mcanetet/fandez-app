...
const TEMPLATES = {
  ...
  'payment.voucher': (ctx) => ({
    ...
    html: `
      ...
      <p><a href="${ctx.voucherUrl}" style="display:inline-block;background:#0D9488;color:#fff;text-decoration:none;padding:10px 16px;border-radius:10px;font-weight:600">Ver comprobante</a></p>
  ...
  'service.no_provider': (ctx) => ({
    ...
    text: `Hola ${ctx.client?.name || 'cliente'},\n\nHan pasado 15 minutos y todavía no hemos encontrado un socio para tu servicio de ${ctx.request.serviceName}.\n\n¿Quieres que sigamos buscando?\n\n1. Sí, seguir buscando: ${ctx.continueUrl}\n2. Solicitar devolución (siguiente día hábil): ${ctx.refundUrl}\n\nTambién puedes elegir desde la app de Fundez.`,
    html: `
      <p>Hola <strong>${ctx.client?.name || 'cliente'}</strong>,</p>
      <p>Han pasado <strong>15 minutos</strong> y todavía no hemos encontrado un socio para tu servicio de <strong>${ctx.request.serviceName}</strong>.</p>
      <p><strong>¿Quieres que sigamos buscando?</strong></p>
      <p style="margin:20px 0">
        <a href="${ctx.continueUrl}" style="display:inline-block;background:#0D9488;color:#fff;text-decoration:none;padding:11px 16px;border-radius:10px;font-weight:600;margin:0 8px 8px 0">Sí, seguir buscando</a>
  ...
  'technician.en_route': (ctx) => ({
    ...
    html: `
      ...
      <p style="margin:22px 0">
        <a href="${trackUrl(ctx.request)}" style="display:inline-block;background:#0D9488;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700">Ver en el mapa</a>
      </p>
      <p style="color:#6B635A;font-size:13px">También puedes seguirlo desde la app Fundez, como en Uber o PedidosYa.</p>
    `
  }),
  ...
  'dte.issued': (ctx) => {
    ...
    return {
      ...
      html: `
        ...
        <p><a href="${link}" style="display:inline-block;background:#0D9488;color:#fff;text-decoration:none;padding:10px 16px;border-radius:10px;font-weight:600">Descargar documento</a></p>
