...
const TEMPLATES = {
  ...
  'technician.job_assigned': (ctx) => {
    ...
    const subject = `Te asignaron un pedido — ${service}`;
    const text = [
      `Hola ${techName},`,
      '',
    ...
    return {
      ...
      html: transactional({
        ...
        heading: 'Te asignaron un pedido',
        greeting: `Hola ${techName},`,
        intro: `Tu socio te asignó una visita de ${service}. Ábrela y acéptala en la app (tienes ${mins} minutos).`,
        details: [
  ...
  'technician.invited': (ctx) => {
    ...
    const verifyUrl = ctx.verifyUrl || `${appBaseUrl()}/verificar-email`;
    ...
    const text = [
      ...
      '',
      `1) Entra en ${loginUrl} con tu correo ${email}.`,
      '2) Usa la contraseña que te indicó tu empresa.',
      code
        ? `3) Verifica tu correo con este código (válido 15 min): ${code}`
        : '3) Si te pedimos verificar el correo, revisa tu bandeja o solicita el código en la app.',
      '',
      `Panel técnico: ${technicianPanelUrl()}`,
      '',
    ...
    return {
      ...
      html: transactional({
        ...
        greeting: `Hola ${techName},`,
        intro: `${companyName} te agregó como técnico. Entra con el correo de esta invitación y la contraseña que te dio tu empresa.`,
        code: code || undefined,
        details,
        cta: { href: code ? verifyUrl : loginUrl, label: code ? 'Verificar correo' : 'Iniciar sesión' },
...
async function sendEvent(event, ctx) {
  ...
  const isTechMail = event === 'technician.job_assigned' || event === 'technician.invited';
  return notify({
    event,
