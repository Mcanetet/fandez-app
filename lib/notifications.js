...
const TEMPLATES = {
  ...
  'technician.invited': (ctx) => {
    ...
    const text = [
      `Hola ${techName},`,
      '',
      `${companyName} te invitó a unirte como técnico en Fandez.`,
      '',
      `Abre este enlace para crear tu contraseña y completar tu registro:`,
      activateUrl,
      '',
      `Correo de tu cuenta: ${email}`,
      '',
    ...
    const details = [
      ...
      { label: 'Crear cuenta', value: activateUrl }
    ];
    return {
      subject,
      text,
      html: transactional({
        title: subject,
        preheader: `${companyName} te invita a su equipo en Fandez`,
        eyebrow: 'Invitación a equipo',
        heading: 'Te invitaron a Fandez',
        greeting: `Hola ${techName},`,
        intro: `${companyName} te invitó como técnico. Abre el enlace, crea tu contraseña y completa tu ficha (carnet).`,
        details,
        cta: { href: activateUrl, label: 'Crear mi cuenta' },
