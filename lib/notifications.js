...
const TEMPLATES = {
  ...
  'materials.purchase_proposed': (ctx) => {
    const subject = `OK de materiales — ${ctx.request.serviceName}`;
    const name = clientName(ctx);
    const text = `Hola ${name},\n\nEl técnico necesita comprar material adicional (~${formatCLP(ctx.amount)}):\n${ctx.description || 'Material'}\n\nDebes aprobar o rechazar en la app:\n${trackUrl(ctx.request)}\n\nFandez`;
    return {
      subject,
      text,
      html: transactional({
        title: subject,
        preheader: `Material adicional ~${formatCLP(ctx.amount)}`,
        eyebrow: 'Materiales',
        heading: 'El técnico pide OK para comprar',
        greeting: `Hola ${name},`,
        intro: 'Si el material es de $10.000 o más, se cobra aparte (a costo). Materiales menores a $10.000 van incluidos en el servicio.',
        highlight: { label: 'Estimado', value: formatCLP(ctx.amount), tone: 'warning' },
        details: [
  ...
  'materials.purchase_resolved': (ctx) => {
    const subject = `Materiales ${ctx.approved ? 'aprobados' : 'rechazados'} — ${ctx.request.serviceName}`;
    const text = `Hola ${ctx.provider?.name || ctx.request.technicianName || 'equipo'},\n\nEl cliente ${ctx.approved ? 'aprobó' : 'rechazó'} la compra de material (~${formatCLP(ctx.amount)}).\n${ctx.approved ? 'Puedes ir a comprar y debes subir la factura/boleta al pedido.\n' : 'Continúa sin ese material o propone otra opción.\n'}Dirección: ${ctx.request.address}`;
    return {
      subject,
      text,
      html: transactional({
        title: subject,
        preheader: ctx.approved ? 'Cliente aprobó materiales' : 'Cliente rechazó materiales',
        eyebrow: ctx.approved ? 'OK materiales' : 'Rechazo materiales',
        heading: ctx.approved ? 'Ve a comprar y sube la factura' : 'Material no aprobado',
        greeting: `Hola ${ctx.provider?.name || ctx.request.technicianName || 'equipo'},`,
        intro: ctx.approved
          ? 'El cliente aprobó. Ve a comprar el material adicional y sube la boleta/factura al pedido.'
          : 'El cliente rechazó la compra. Continúa sin ese material o acuerda otra vía.',
        details: [
...
async function sendEvent(event, ctx) {
  ...
  return notify({
    event,
    to: ctx.to || client?.email || provider?.email,
    phone: client?.phone || provider?.phone || ctx.phone,
    subject,
    text,
    html,
    requestId: ctx.request?.id,
    userId: ctx.userId || client?.id || provider?.id,
    meta: ctx.meta || {}
  });
}
