...
const SEED_MODULES = [
  ...
  { id: 'client_whatsapp', audience: 'client', name: 'Concierge (legado)', description: 'Solo si Aland IA está OFF: botón de soporte clásico', sortOrder: 10, enabled: false },
  { id: 'client_aland', audience: 'client', name: 'Chat Sofía', description: 'Asistente de soporte: primero IA, luego socio o pagos', sortOrder: 11, enabled: true },
...
const SEED_USERS = [
  ...
  },
  {
    ...
    specialties: ['termos', 'gasfiter', 'otros'],
    rating: 4.9,
    reviewsCount: 67,
    online: false,
    avatar: 'AR',
    bio: 'Técnica certificada en electrodomésticos y termos. Hidráulica vía gasfitería; electrónica vía Otros.',
    reviews: [
...
function userToRow(user) {
  return {
    ...
    password_reset_sent_at: user.passwordResetSentAt || null,
...
async function ensureDemoServices() {
  const forceDisable = new Set(['lavadora', 'lavavajillas']);
  for (const service of SEED_SERVICES) {
    await saveService(service, { preserveEnabled: !forceDisable.has(service.id) });
  }
}
